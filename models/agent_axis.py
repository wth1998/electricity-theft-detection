...
class H3_Agent_AXIS(nn.Module):
    ...
    def __init__(self, llm_path, perception_layer=None, load_in_4bit=False, config=None):
        super().__init__()
    ...
    def construct_prompt(self, axis_hints, user_instructions, ground_truth=None):
        ...
        for i, (hint, instr) in enumerate(zip(axis_hints, user_instructions)):
            ...
            if ground_truth is not None:
                ...
                text = prompt_text + assistant_content + "<|im_end|>\n"
            else:
    ...
    def forward(self, batch_data, user_instructions, labels=None, batch_idx=None, debug=False):
        """
        前向传播
        ...
        if debug and batch_idx is not None and batch_idx == 0:
            ...
            DebugLogger.log_tensor("LLM词嵌入", word_embeddings, 
                                   print_shape=True, print_stats=True, print_sample=False)
        
        # 2. 感知层处理 -> 获取 Soft Prompts
        soft_prompts, perception_info = self.perception(batch_data, llm_embeds=word_embeddings)
        ...
        # 5. 构建 Attention Mask
        prefix_mask = torch.ones((batch_size, soft_prompt_len), dtype=tokens.attention_mask.dtype, device=self.llm.device)
        attention_mask = torch.cat([prefix_mask, tokens.attention_mask], dim=1)
        
        # 6. 训练模式：计算Loss
        if labels is not None:
            # 构建 prefix labels (Soft Prompts 的部分不计算 Loss)
            prefix_labels = torch.full(
                (batch_size, soft_prompt_len), # 去掉了之前的 1+
                -100,
            ...
            # 拼接最终labels (直接拼接，不再切片)
            final_labels = torch.cat([prefix_labels, text_labels], dim=1)
    ...
    @torch.no_grad()
    def generate(self, batch_data, user_instructions, max_new_tokens=20, return_scores=True, debug=False, labels=None):
        """
        推理生成函数
        ...
        if debug:
            from utils.debug_utils import DebugLogger
            DebugLogger.log_tensor("LLM输入 embeddings", inputs_embeds, sample_indices=(0,))
            ...
            print(text_prompts[0][:800] + "..." if len(text_prompts[0]) > 800 else text_prompts[0])
            print(f"{'='*60}\n")
        ...
        if return_scores:
            ...
            if debug:
                ...
                for i in range(min(3, len(response_texts_raw))):
                    ...
                    print(f"  样本{i}: 原始='{raw_short}...' -> 清理='{clean_short}...'")
            ...
            for i in range(batch_size):
                ...
                if first_gen_logits is not None:
                    ...
                    if debug and i == 0:
                        ...
                        for tid in theft_token_ids:
                            if tid < len(probs):
                                token_str = self.tokenizer.decode([tid]) if tid < len(self.tokenizer) else "<?>"
                                print(f"      ID={tid} '{token_str}': {probs[tid].item():.4f}")
                    ...
                    # 简化逻辑：取所有候选中的最大概率
                    normal_candidates = [tid for tid in normal_token_ids if tid < len(probs)]
                    if normal_candidates:
                        normal_prob = max([probs[tid].item() for tid in normal_candidates])
                    
                    theft_candidates = [tid for tid in theft_token_ids if tid < len(probs)]
                    if theft_candidates:
                        # 排除过于通用的token如换行符(198)
                        filtered_candidates = [tid for tid in theft_candidates if tid != 198]
                        if filtered_candidates:
                            theft_prob = max([probs[tid].item() for tid in filtered_candidates])
                        else:
                            theft_prob = max([probs[tid].item() for tid in theft_candidates])
                ...
                elif re.search(r'\b(abnormal|suspicious|fraud|steal)\b', response_text, re.IGNORECASE):
                ...
                if debug and i < min(batch_size, 5):  # 打印最多5个样本
                    label_str = f"[真实: {'Theft' if labels[i] == 1 else 'Normal' if labels is not None else 'Unknown'}]" if labels is not None else ""
                    ...
                    if first_gen_logits is not None:
                        ...
                        for j, (prob, idx) in enumerate(zip(top5_probs.tolist(), top5_indices.tolist())):
                            token_str = self.tokenizer.decode([idx]) if idx < len(self.tokenizer) else "<?>"
                            marker = " <-- Theft" if idx in theft_token_ids else " <-- Normal" if idx in normal_token_ids else ""
                            print(f"    {j+1}. '{token_str}' (ID={idx}): {prob:.4f}{marker}")
                    else:
                    ...
                    print(f"  预测结果: {'Theft' if final_theft_score > 0.5 else 'Normal'}")
                    print(f"{'-'*60}")
            ...
            if debug:
                ...
                DebugLogger.log(f"批次预测统计: 均值={scores_arr.mean():.4f}, 范围=[{scores_arr.min():.4f}, {scores_arr.max():.4f}]")
