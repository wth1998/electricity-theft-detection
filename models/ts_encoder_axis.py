...
class TimeSeriesEncoderAXIS(nn.Module):
    ...
    def forward(self, time_series, mask=None):
        ...
        local_embeddings = local_embeddings.permute(0, 1, 2, 3, 4)  # (B, num_patches, patch_size, num_features, d_proj)
        local_embeddings = local_embeddings.reshape(B, -1, self.num_features, self.d_proj)
...
class PretrainHeads(nn.Module):
    ...
    def forward(self, local_embeddings):
        ...
        anomaly_logits = self.anomaly_head(local_embeddings).mean(dim=2)  # (B, seq_len, 2)
