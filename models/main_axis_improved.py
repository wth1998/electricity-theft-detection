...
class ElectricityDatasetAXIS(Dataset):
    ...
    def _generate_enhanced_analysis(self, index):
        ...
        if feat["zero_ratio"] > 0.3:
            risk_signals.append("high zero")
        if feat["constant_ratio"] > 0.5:
            risk_signals.append("stable pattern")
        if abnormal_count > 10:
            risk_signals.append("frequent anomalies")
        if feat["night_ratio"] < 0.05 and not is_weekend:
            risk_signals.append("low night usage")

        risk_str = " | ".join(risk_signals) if risk_signals else "none"

        # 【优化】一行关键信息替代多行详细统计
        analysis = (
            f"[{season} {day_type}] "
            f"Usage: {feat['mean']:.2f}kWh (hist:{u_stats['mean']:.2f}), "
            f"Zero:{feat['zero_ratio'] * 100:.0f}%, "
            f"Stable:{feat['constant_ratio'] * 100:.0f}%, "
            f"Anomaly:{abnormal_count}, "
            f"Risks:{risk_str}"
        )
    ...
    def __getitem__(self, idx):
        ...
        u_stats = self.user_stats.get(
            uid, {"mean": np.mean(vals), "std": np.std(vals) + 1e-6}
        )
        ...
        # 转换为tensor时使用标准化后的数据
        x_seq_tensor = torch.tensor(vals_normalized, dtype=torch.float32).unsqueeze(
            -1
        )  # (seq_len, 1)

        result = {"series": x_seq_tensor, "text": text_hint}
        ...
        if idx == 0 and hasattr(self, "_debug_printed") is False:
            ...
            DebugLogger.log(
                f"  原始值范围: [{vals.min():.3f}, {vals.max():.3f}], 当天均值: {np.mean(vals):.3f}"
            )
            DebugLogger.log(f"  用户历史均值: {mean:.3f}, 历史标准差: {std:.4f}")
            DebugLogger.log(
                f"  标准化后: 均值={vals_normalized.mean():.6f}, 标准差={vals_normalized.std():.6f}"
            )
...
class NumericalStreamAXIS(nn.Module):
    ...
    def forward(self, x, mask=None, return_all_features=False, debug=False):
        ...
        if debug:
            ...
            DebugLogger.log_tensor(
                "NumericalStream输出", local_embeddings, sample_indices=(0,)
            )
    ...
    def enable_pretrain_heads(self):
        ...
        self.pretrain_heads = PretrainHeads(
            d_proj=self.d_proj, num_features=1, dropout=0.1
        )
