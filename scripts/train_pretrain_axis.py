...
class ElectricityDatasetPretrain(ElectricityDatasetAXIS):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        super().__init__(
            csv_file, user_stats_path, mode="train", for_pretrain=False, **kwargs
        )
    ...
    def __getitem__(self, idx):
        ...
        # 生成伪异常标签（用于辅助任务）
        # 【修复】使用用户历史统计量而非当日统计，避免低用电量用户的误判
        ...
        z_scores = np.abs((vals - historical_mean) / historical_std)
...
def pretrain_model(
    ...
    scheduler = CosineAnnealingWarmRestarts(optimizer, T_0=epochs // 4, T_mult=2)
    ...
    for epoch in range(epochs):
        ...
        checkpoint_name = (
            f"pretrain_{model_config_name.lower()}_ep{epoch}_loss{avg_loss:.4f}.pth"
        )
        ...
        if avg_loss < best_loss:
            ...
            torch.save(
                checkpoint,
                f"{checkpoint_dir}/pretrain_{model_config_name.lower()}_best.pth",
            )
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--mask-ratio", type=float, default=0.15, help="Mask ratio for pretraining"
    )
    parser.add_argument(
        "--recon-weight", type=float, default=1.0, help="Weight for reconstruction loss"
    )
    parser.add_argument(
        "--anomaly-weight",
        type=float,
    ...
    parser.add_argument(
        "--checkpoint-dir",
        type=str,
        default="checkpoints_pretrain_axis",
        help="Checkpoint directory",
    )
    parser.add_argument(
        "--quick", action="store_true", help="Quick mode: 1000 samples, 10 epochs"
    )
    parser.add_argument(
