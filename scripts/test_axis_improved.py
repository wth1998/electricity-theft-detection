...
from models.main_axis_improved import H3_PerceptionLayerAXIS, ElectricityDatasetAXIS, ModelConfig
...
class ElectricityDatasetWithLabel(ElectricityDatasetAXIS):
    ...
    def __init__(self, csv_file, user_stats_path, max_users=None, max_samples=None, **kwargs):
        super().__init__(csv_file, user_stats_path, mode='test', for_pretrain=False, **kwargs)
        ...
        if max_samples is not None:
            ...
            self.dates = pd.to_datetime(self.df.iloc[:, 1])
        elif max_users is not None:
            unique_users = self.df.iloc[:, 0].unique()
            selected_users = unique_users[:max_users]
            print(f"[快速测试] 加载前 {max_users} 个用户的数据")
            
            mask = self.df.iloc[:, 0].isin(selected_users)
...
def run_inference(agent, max_users=None, max_samples=None, output_dir='results_axis_improved', debug=False):
    """
    运行推理
    ...
    for batch_idx, batch in enumerate(loader):
        ...
        instruction = ["Analyze this user's electricity usage pattern."] * current_batch_size
        ...
        responses, _, theft_scores = agent.generate(
            batch, instruction, return_scores=True, 
            debug=batch_debug, labels=ground_truths if batch_debug else None
        )
        ...
        for i in range(current_batch_size):
            ...
            if "theft" in lower_res and "normal" not in lower_res:
                ...
                # 取最后出现的词
                pred_class = "Theft" if lower_res.rfind("theft") > lower_res.rfind("normal") else "Normal"
            else:
                # 如果无法解析，根据分数判断
                pred_class = "Theft" if theft_score > 0.5 else "Normal"
            
            # 置信度
            confidence = "High" if theft_score > 0.8 or theft_score < 0.2 else \
                        "Medium" if theft_score > 0.6 or theft_score < 0.4 else "Low"
            
            if (global_user_idx + 1) % 100 == 0:
                print(f"  [User {global_user_idx}] True: {true_label_text} | "
                      f"Pred: {pred_class} | Score: {theft_score:.3f}")
            
            metrics.update(true_label_text, pred_class, theft_score, user_id=global_user_idx)
    ...
    print(f"\n【自适应阈值分析】")
    print(f"默认阈值 (0.5) 的F1: {f1_score(all_labels, [1 if s > 0.5 else 0 for s in all_scores], zero_division=0):.4f}")
    print(f"最优阈值 ({best_thresh:.2f}) 的F1: {best_f1:.4f}")
    
    # 使用最优阈值重新计算预测
    for r in results_list:
        r['Predicted_Class_Adjusted'] = 'Theft' if r['Score'] > best_thresh else 'Normal'
    ...
    for k in [40, 100, 200]:
        ...
        correct_in_topk = sum(1 for item in topk if item['correct'] and item['true_label'] == 'Theft')
...
def main():
    ...
    parser.add_argument('--max-users', type=int, default=None,
                       help='Limit test users for quick test')
    parser.add_argument('--max-samples', type=int, default=None,
                       help='Limit test samples for quick test')
    parser.add_argument('--quick', action='store_true',
                       help='Quick test mode: test 1000 samples only')
    parser.add_argument('--output-dir', type=str, default='results_axis_improved',
