...
| 参数 | 说明 | 默认值 |
|------|------|--------|
| `--config` | 模型配置 [SMALL/BASE/MEDIUM/LARGE] | MEDIUM |
| `--use-first-n` | 使用样本数 | 5000 |
| `--seed` | 随机种子 | 42 |
...
| 参数 | 说明 | 推荐值 |
|------|------|--------|
| `--pretrain-ckpt` | 预训练权重路径 | auto |
| `--batch-size` | 批次大小 | 4 (2x3090) |
| `--lr` | 学习率 | 2e-4 |
| `--seed` | 随机种子 | 42 |
...
### 📝 2. 精简文本提示
```
从 ~150 tokens 减少到 ~50 tokens
保留关键指标：用电量、零值比例、稳定模式、异常数、风险信号
```

### 📊 3. 验证集指标监控
- AUC、MAP@40、WF1、F1_Theft
- 综合评分用于早停 (AUC 30% + MAP@40 30% + WF1 20% + F1_Theft 20%)
...
| 配置 | d_model | d_proj | Layers | Heads | 参数量 | 场景 |
|------|---------|--------|--------|-------|--------|------|
...
**Q: 损失不下降？**
> A: 检查：1) Label是否正确对齐 2) 随机种子是否设置 3) 学习率是否过高

**Q: 显存不足？**
> A: 使用 `--config SMALL`，或减小 `--batch-size`，或启用 `load_in_4bit=True`

**Q: 结果不稳定？**
> A: 确保使用相同的 `--seed`

**Q: 窃电召回率低？**
> A: 尝试：1) 降低阈值 2) 增加训练轮数 3) 使用Focal Loss
...
<p align="center">
  <strong>最后更新</strong>: 2025年2月  |  <strong>版本</strong>: v2.0 (修复版)
</p>
